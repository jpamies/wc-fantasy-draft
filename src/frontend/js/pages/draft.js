...
Router.register('#/draft', async (container) => {
    ...
    function normalizePosition(pos) {
        ...
        if (['GK', 'GKP', 'GOALKEEPER'].includes(p)) return 'GK';
        if (['DEF', 'DF', 'D', 'DEFENDER'].includes(p)) return 'DEF';
        if (['MID', 'MF', 'M', 'MIDFIELDER'].includes(p)) return 'MID';
        if (['FWD', 'FW', 'ATT', 'ST', 'CF', 'FORWARD', 'STRIKER'].includes(p)) return 'FWD';
        return p;
    }
    ...
    function render() {
        ...
        const myPicks = (state.picks || [])
            ...
            myPicks.forEach(p => {
                ...
                if (myPosCounts[pos] !== undefined) myPosCounts[pos] += 1;
            });
            ...
            <div class="player-card draft-player-card" data-pid="${p.id}">
                <div class="draft-player-top">
                    ...
                    <div class="draft-player-meta">
                        ${posBadge(p.position)}
                        <span class="player-ovr">OVR ${p.strength || '-'}</span>
...
async function renderRepositionDraft(container, leagueId, windowId) {
    ...
    async function loadTeam() {
        try { team = await API.get(`/teams/${teamId}`); } catch { team = { players: [] }; }
    }
                        ...
                        ${['GK','DEF','MID','FWD'].map(pos => {
                            ...
                            return `<div style="margin-bottom:.5rem">
                                ...
                                ${pp.map(p => `<div style="font-size:.78rem;padding:.1rem 0;display:flex;align-items:center;gap:.4rem">
                                    ${posBadge(pos)} <span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.name}</span>
                                    <span style="color:var(--text-muted);font-size:.7rem">${p.country_code}</span>
                                </div>`).join('')}
                            </div>`;
                        }).join('')}
                        ${(team.players || []).length === 0 ? '<p style="color:var(--text-muted);font-size:.85rem">Sin jugadores</p>' : ''}
                    </div>
                </div>
            </div>
        `;
    ...
    function renderRepoPlayerList(list, canPick, myPosCounts = { GK: 0, DEF: 0, MID: 0, FWD: 0 }) {
        ...
        return list.map(p => `
            <div class="player-card" data-pid="${p.id}">
                ...
                <div class="player-info">
                    <div class="player-name"><a href="#/player/${p.id}" style="color:inherit;text-decoration:none">${p.name}</a></div>
                    <div class="player-meta">${p.country_code}${p.club ? ' · ' + p.club : ''}</div>
                </div>
                ${posBadge(p.position)}
                ...
                ${canPick ? `<button class="btn btn-primary btn-sm repo-pick-btn" data-pid="${p.id}" title="${(myPosCounts[normalizePosition(p.position)] || 0) >= 4 ? 'Tope de posicion alcanzado (4)' : ''}" ${(myPosCounts[normalizePosition(p.position)] || 0) >= 4 ? 'disabled' : ''}>${(myPosCounts[normalizePosition(p.position)] || 0) >= 4 ? 'Tope' : 'Pick'}</button>` : ''}
            </div>
        `).join('');
    }
